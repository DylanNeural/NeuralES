...
class OrganisationRepositorySQL(OrganisationRepository):
    ...
    def create(self, org: Organisation) -> Organisation:
        m = OrganisationModel(
            ...
            adresse=org.adresse,
            created_at=org.created_at,  # fourni par le use case
        )
        self.db.add(m)
        self.db.commit()
        self.db.refresh(m)
    ...
    def list(self, limit: int, offset: int) -> list[Organisation]:
        stmt = select(OrganisationModel).order_by(OrganisationModel.organisation_id).limit(limit).offset(offset)
    ...
    def delete(self, organisation_id: int) -> bool:
        stmt = delete(OrganisationModel).where(OrganisationModel.organisation_id == organisation_id)
